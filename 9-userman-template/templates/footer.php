<!-- kontak -->
<section class="kontak">

    <div class="container py-5 ">
        <h1 class="text-center font-title mb-5 text-primary">Kontak Kami</h1>

        <div class="row justify-content-center g-3 mb-5">
            <div class="col-md-3">
                <div class="card shadow text-center">
                    <div class="card-body">
                        <h1 class="text-primary"><i class="fa-solid fa-envelope"></i></h1>
                        <p class="card-text my-3">userman.id</p>
                    </div>
                </div>
            </div>
            <div class="col-md-3">
                <div class="card shadow text-center">
                    <div class="card-body">
                        <h1 class="text-primary"><i class="fa-solid fa-phone"></i></h1>
                        <p class="card-text my-3">0411-8888888</p>
                    </div>
                </div>
            </div>
            <div class="col-md-6">
                <div class="card shadow text-center">
                    <div class="card-body">
                        <h1 class="text-primary"><i class="fa-solid fa-map"></i></h1>
                        <p class="card-text my-3">Lorem ipsum dolor sit amet consectetur, adipisicing elit. Asperiores, totam!</p>
                    </div>
                </div>
            </div>
        </div>

    </div>

    <iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d4725.720634905247!2d119.49641823927188!3d-5.1292127560423895!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x2dbefd6afd5eb049%3A0x29adcc881e289c72!2sUNITAMA!5e0!3m2!1sid!2sid!4v1671789961436!5m2!1sid!2sid" width="600" height="450" style="border:0;" allowfullscreen="" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe>

</section>
<!-- end kontak -->

<!-- footer -->
<footer class="bg-primary ">
    <div class="container py-3 text-center fw-bold text-white">
        &copy; 2022 USER MANANER TEMPLATE
    </div>
</footer>
<!-- end footer -->

<script src="<?= $base_url; ?>assets/plugins/jquery/jquery-3.6.3.min.js"></script>
<script src="<?= $base_url; ?>assets/plugins/bootstrap/bootstrap.js"></script>

<!-- datatables -->
<script src="<?= $base_url; ?>assets/plugins/datatables/jquery.dataTables.min.js"></script>
<script src="<?= $base_url; ?>assets/plugins/datatables/dataTables.bootstrap5.min.js"></script>
<script src="<?= $base_url; ?>assets/plugins/datatables/plugin/dataTables.buttons.min.js"></script>
<script src="<?= $base_url; ?>assets/plugins/datatables/plugin/buttons.flash.min.js"></script>
<script src="<?= $base_url; ?>assets/plugins/datatables/plugin/jszip.min.js"></script>
<script src="<?= $base_url; ?>assets/plugins/datatables/plugin/pdfmake.min.js"></script>
<script src="<?= $base_url; ?>assets/plugins/datatables/plugin/vfs_fonts.js"></script>
<script src="<?= $base_url; ?>assets/plugins/datatables/plugin/buttons.html5.min.js"></script>
<script src="<?= $base_url; ?>assets/plugins/datatables/plugin/buttons.print.min.js"></script>
<!-- end datatables -->

<script src="<?= $base_url; ?>assets/plugins/ckeditor/ckeditor.js"></script>
<script src="<?= $base_url; ?>assets/js/source.js"></script>

</body>

</html>